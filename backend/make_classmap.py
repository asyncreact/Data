...
def extract_fors_from_html(text):
    fors = set()
    for m in re.finditer(r'\bfor\s*=\s*([\'"])(.*?)\1', text, flags=re.I | re.S):
        v = m.group(2).strip()
        if v:
            fors.add(v)
    return fors
...
# 2) Mapeo único: mismo original => mismo random
name_map = {}
used = set()
...
def build_mapping(items):
    return {k: get_or_create(k) for k in sorted(items)}

data = {
    "classes": build_mapping(classes),
    "ids": build_mapping(ids),
    # Importante: for debe mapear igual que id cuando coinciden
    "fors": build_mapping(fors),
}
...
print(
    f"Clases: {len(data['classes'])} | IDs: {len(data['ids'])} | FORs: {len(data['fors'])} -> map.json"
)
